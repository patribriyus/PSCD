...
int main() {
    ...
    //Si la invocación al compilador contiene "-D DEBUG_MODE=true", se ejecuta esta parte
    //Si no, la del else
    #if DEBUG_MODE
        Logger LOG("elLog.log", cerr); //generará el fichero de log "elLog.log"
        Semaphore s1(1, &LOG, "s1");   //usa tercer constructor
        Semaphore s2(&LOG, "s2");      //usa cuarto constructor
    #else
    ...
    Semaphore* r[2];   //arra de dos punteros a semáforo. De momento, no usa constructor

#!/bin/bash
#*****************************************************************
# File:   pract4_correcta.bash
# Author: PSCD-Universidad de Zaragoza
# Date:   octubre 2018
# Coms:   script de comprobación de entrega de la práctica 4 de PSCD (18/19)
#         Funcionamiento: 
#          1) Dar permisos de ejecución al script: "chmod u+x pract4_correcta.bash"
#          2) Ejecutar mediante "pract4_correcta.bash miNip" en el directorio donde esté el zip
#             a entregar, en este caso "practica4_miNip.zip". Creará un directorio temporal
...
DIR_CORRECCION=$(pwd)
MAKE_FILE=Makefile_p4_e1
EJECUTABLE=main_p4_e1
FUENTE_ZIP="practica4_${1}.zip"
USER=a$1       #usuarios en Hendrix son "a" seguido del NIP
FALTAN=()      #ficheros que faltan
FICHEROS=(ControlGasolinera/ControlGasolinera.cpp \
	      ControlGasolinera/ControlGasolinera.hpp \
	      ControlGasolinera/$EJECUTABLE.cpp \
	      informe_p4.pdf)
DIRECTORIOS=(ControlGasolinera Logger)
...
DIR_COMPILACION=$DIR_TEMPORAL/ControlGasolinera
...
if [[ "$#" -ne 1 ]]
then
	echo "Invocar como: pract4_correcta.sh miNip"
	echo "------------------------------------------"
	exit
fi
#----------------------------------------------------------------
